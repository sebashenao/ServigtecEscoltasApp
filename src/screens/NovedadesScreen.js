...
const NovedadesScreen = ({ navigation, route }) => {
    ...
    const [tiposNovedad, setTiposNovedad] = React.useState([]);
    ...
    React.useEffect(async () => {
        ...
        setTiposNovedad(route.params.tiposNovedad);
    ...
    const abrirCamara = async () => {
        ...
        if (fotos.length === 10) {
            showToastWithGravityAndOffset('No puede agregar mas de 10 fotos a la novedad');
        }
        else {
    ...
    const upload = async () => {
        ...
        try {
            var text = '';
            fotos?.length == 0 && (text += '\nDebe agregar al menos 1 foto a la novedad.');
            audio?.length == 0 && (text += '\nDebe agregar al menos 1 audio a la novedad');
            direccion?.length == 0 && (
            ...
            tipo?.length == 0 && (text += '\nCampo tipo novedad es requerido.');
            ...
            if (text?.length > 0) {
                ...
                data.push({ name: 'tipo_id', data: `${tipo}` });
                ...
                RNFetchBlob.fetch('POST', `${environment.api}/novedad`, {
                    ...
                    if (json.error) {
                        ...
                        showToastWithGravityAndOffset('Novedad almacenada con exito.');
                        navigation.goBack()
                    }
                    setIsLoading(false)
    ...
    return (
        <View style={styles.container}>
            <Spinner
                visible={isLoading}
                textContent={'Almacenando Novedad, espere un momento...'}
                textStyle={styles.spinnerTextStyle}
                overlayColor='rgba(0, 0, 0, 0.7)'
            />
            <Text style={{ fontSize: 18, fontWeight: 'bold' }}>Tipo Novedad</Text>
            <Picker
                mode="dropdown"
                selectedValue={tipo}
                style={{ flexDirection: 'row' }}
                onValueChange={(itemValue, itemIndex) => setTipo(itemValue)}>
                <Picker.Item label="Seleccione Tipo Novedad" value=""></Picker.Item>
                {
                    tiposNovedad?.map((tipo) =>
                        <Picker.Item label={tipo.nombre} value={tipo.id} key={tipo.id}></Picker.Item>
                    )
                }
            </Picker>
            <Text style={{ fontSize: 18, fontWeight: 'bold' }}>Observacion</Text>
            <TextInput
